...
/**
 * Class that imitates Unix commands for a file system. ls, pwd, cd, cd .., cd ., cd <dir>, cp, mv,
 * rm, mkdir, rmdir, exit
 * @author Leta
 * 
 */
public class CommandParser {
   ...
   private static void parseInput(String input) throws IOException {
      ...
      case "ls":
         ...
         break;
      default:
         bs.write("Invalid command.".getBytes());
         bs.flush();
         break;
      }
   }
   ...
   private static void mkdir(String arg) throws IOException {
      ...
      for (int i = 0; i < newDirectories.length; i++) {
         ...
         if (d == null) {
            bs.write(("Invalid path. Could not make directory '" + newDirectories[i] + "'.")
                  .getBytes());
         } else {
